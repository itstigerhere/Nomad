...
function getTripIcon(trip: Trip): string {
  ...
  if (trip.interest === 'ADVENTURE') return '🧗';
...
const TripsPage: React.FC = () => {
  ...
  async function fetchTrips() {
    ...
    try {
      ...
      setEnrolledTrips(data.filter((t: Trip) => t.status === "PLANNED" || t.status === "IN_PROGRESS" || t.status === "ENROLLED"));
  ...
  async function handleDelete(tripId: number, e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
        ...
        {!loading && !error && (
          <>
              ...
              {enrolledTrips.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {enrolledTrips.map((trip) => {
                    ...
                    return (
                      <div key={trip.tripRequestId} className="rounded-2xl p-6 transition-all hover:shadow-lg" style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)' }}>
                        <div className="flex items-center justify-between gap-4">
                          <Link href={`/trip-summary?tripId=${trip.tripRequestId}`} className="flex items-center gap-4 flex-1">
                            <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl" style={{ background: 'rgba(97, 194, 162, 0.1)' }}>
                              {getTripIcon(trip)}
                            </div>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              onClick={(e) => handleDelete(trip.tripRequestId, e)}
            ...
            <div>
              ...
              {pastTrips.length === 0 ? (
                <div className="rounded-2xl p-8 text-center" style={{ border: '2px dashed var(--color-border)' }}>
                  <p style={{ color: 'var(--color-text)', opacity: 0.6 }}>No past trips</p>
                </div>
              ) : (
                <div className="grid gap-4">
                  {pastTrips.map((trip) => {
                    const statusStyle = getStatusColor(trip.status);
                    return (
                      <Link key={trip.tripRequestId} href={`/trip-summary?tripId=${trip.tripRequestId}`}>
                        <div className="rounded-2xl p-6 transition-all hover:shadow-md cursor-pointer" style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', opacity: 0.8 }}>
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4 flex-1">
                              <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl" style={{ background: 'rgba(0,0,0,0.05)' }}>
                                {getTripIcon(trip)}
                              </div>
                              <div className="flex-1">
                                <h3 className="font-bold text-lg mb-1" style={{ color: 'var(--color-text)' }}>{getTripName(trip)}</h3>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <svg className="w-6 h-6" style={{ color: 'var(--color-text)', opacity: 0.3 }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
